...
/*
un albero binario di ricerca a un qualsisi albero che deve rispettare la proprieta di ricerca
la proprita di ricerca dice che per ogni nodo u appartente all'albero deve avere
...
in caso fosse fortemente sbilanciato l'albero avremo un theta(n)
...
proposizione: dato un nodo con 2 figli il suo successore a un nodo che non ha il figlio sx
*/
...
Per predecessore si intende...
proposizione: dato un nodo con 2 figli il suo predecessore a un nodo che non ha il figlio dx
*/
...
/*operazione di utilita Trasplant(T, u, v)
...
//complessita theta(1) in quanto sono solo assegnamenti

//funzione cardine per l'eleminazione
void TreeDelete(PTree T, PNode z) {     //precondizione z a in T
    if (z->left == nullptr) {           //1. z a una foglia oppure un solo nodo a dx
        //utilizzo la trasplant
        Trasplant(T, z, z->right);
    }
    else if (z->right == nullptr) {     //2. z a una foglia oppure un solo nodo a sx
       //utilizzo la trasplant
...
//complessita theta(n^2) per un albero completamente sbilanciato, per esempio l'input a ordinato --> catena!
